...
class Major:
    ...
    def remaining_course(self, cwid, completed) -> tuple:
        ...
        if len(passed) == 0:
            print(f"Student with {cwid} has not passed with minimum grade point")
        else:
...
class Repository:
    ...
    def instructor_table(self) -> None:
        ptable = PrettyTable(field_names=['CWID', 'NAME', 'DEPARTMENT', 'COURSE', 'STUDENTS'])
    ...
    def instructor_table_db(self, db_path: str) -> tuple:
        ...
        query: str = "select s.Name as 'NAME', s.CWID AS 'CWID', g.Course AS 'COURSE', g.Grade AS 'GRADE', i.Name AS 'INSTRUCTOR' from students s, grades g, instructors i where s.CWID = g.StudentCWID and i.CWID = g.InstructorCWID order by s.Name"
        
        for row in db.execute(query):
            yield row
    
    def student_grade_summary_pretty_table(self) -> None:
        ptable = PrettyTable(field_names=['NAME', 'CWID', 'COURSE', 'GRADE','INSTRUCTOR'])

        for i in self.instructor_table_db('C:\\sqlite\\810_startup.db'):
            ptable.add_row(i)
...
def main():
    ...
    Repository('E:\\Stevens Institute of Technology\\2nd Sem\\SSW 810\\Assignments\\HW_11')
